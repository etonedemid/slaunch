...
namespace haze {
    ...
    Result PtpResponder::GetObject(PtpDataParser &dp) {
        ...
        /* sLaunch: report progress to the menu. */
        bool transfer_ok = false;
        TransferBegin(TransferOp_Send, obj->GetName(), size);
        ON_SCOPE_EXIT { TransferEnd(transfer_ok); };
        ...
        while (true) {
            ...
            TransferProgress(offset, bytes_read);
    ...
    Result PtpResponder::SendObject(PtpDataParser &rdp) {
        ...
        /* sLaunch: report progress to the menu. A length of 0xFFFFFFFF means */
        /* the file is over 4GiB and its size is not known up front. */
        bool transfer_ok = false;
        const bool size_known = data_header.length != 0xFFFFFFFF && data_header.length > sizeof(PtpUsbBulkContainer);
        TransferBegin(TransferOp_Receive, obj->GetName(), size_known ? data_header.length - sizeof(PtpUsbBulkContainer) : 0);
        ON_SCOPE_EXIT { TransferEnd(transfer_ok); };
        ...
        while (true) {
            ...
            TransferProgress(offset, bytes_received);
    ...
    Result PtpResponder::DeleteObject(PtpDataParser &dp) {
        ...
        /* sLaunch: report to the menu. */
        bool transfer_ok = false;
        TransferBegin(TransferOp_Delete, obj->GetName(), 0);
        ON_SCOPE_EXIT { TransferEnd(transfer_ok); };
