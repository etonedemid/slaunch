...
namespace haze {
    ...
    Result PtpResponder::GetPartialObject64(PtpDataParser &dp) {
        ...
        /* sLaunch: a piece of a file, so progress through the whole file. */
        TransferBegin(TransferOp_Send, obj->GetName(), file_size);
        ON_SCOPE_EXIT { TransferEnd(false); };
    ...
    Result PtpResponder::SendPartialObject(PtpDataParser &rdp) {
        ...
        /* sLaunch: a piece of a file whose final size is not known. */
        TransferBegin(TransferOp_Receive, obj->GetName(), 0);
        ON_SCOPE_EXIT { TransferEnd(false); };
        ...
        while (true) {
            ...
            TransferProgress(offset, bytes_to_write);
